import _Image from 'next/image';
import Link from 'next/link';
import 'yet-another-react-lightbox/styles.css';
import Lightbox from 'yet-another-react-lightbox';
import { useState } from 'react';
const Image = _Image.default || _Image;
import images from "../../data/production/NYMLorde.json";

export async function getStaticProps(){
  return {props:{images}};
}
export default function Lorde({images}) {
  
  const [isOpen,setIsOpen] = useState(false);
  const [index, setIndex] = useState(0);
  const [loaded, setLoaded] = useState(Array(images.length).fill(false));

  
  return (
    <main className=" pt-32 min-h-screen px-6 py-10 text-gray-900">
      <Link href="/production" className = "font-dropline block px-10 pb-8">
        BACK
      </Link>
      <section className="flex justify-center">
        {images.map((src, i) => (
          <div key={src} >
      <Image
        src={src}
        alt={`gallery-photo-${i}`}
        width={800}
        height={1200}
        className={`w-170 mb-20 w-full cursor-pointer h-auto opacity-0 transition-opacity duration-300 ease-in-out ${loaded[i] ? 'opacity-100' : ''}`}
        onClick={() => {
          setIndex(i);
          setIsOpen(true);
        }}
        onLoad={() =>
          setLoaded(prev => {
            const updated = [...prev];
            updated[i] = true;
            return updated;
          })
        }
      />
      </div>
      ))}
     
      </section>
      <pre className="text-center text-xs font-mono tracking-widest uppercase whitespace-pre-wrap">
        <u>Location:</u> Xfinity Mobile Arena <br></br>
        <u>Photographer:</u> Mark Seliger<br></br>
        <u>Executive Producer:</u> Ruth Levy <br></br>
        <u>Producer:</u> Madi Overstreet<br></br> <br></br>
        <u>Production Assistant:</u> Maria Penalver<br></br> <br></br>
        <u>Digi Tech:</u> Will Foerster<br></br>
        <u>Photo Assistants:</u> Reggie Desilus, Jonas Søndergaard<br></br>
        <u>Magazine:</u> New York Magazine <br></br>
        <u>Subject:</u> Lorde<br></br>
        <u>Wardrobe Stylist:</u> Daniel Edley <br></br>
        <u>Wardrobe Assistant:</u> Lexi Lang <br></br>
        <u>Hair:</u> Eduardo Mendez using Oribe and Virtue Labs at A Frame <br></br>
        <u>Make-up:</u> Maud Laceppe at Home Agency using Augustinus Bader<br></br>
        <u>Post-Production:</u> Rachel Crowe
</pre>


      <Lightbox
        open={isOpen}
        index={index}
        close={() => setIsOpen(false)}
        slides={images.map(p => ({ src: p }))}
        styles={{
          container: {
            backgroundColor:'#FAF5ED' 
          }
        }}
      />
    </main>

  );
}